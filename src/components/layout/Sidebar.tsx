...
const navItems = [
  ...
  { label: "Sugerencias", icon: MessageSquarePlus, action: "suggestions" },
...
export function Sidebar() {
  ...
  return (
    <>
              ...
              <button
                className={cn(
                  ...
                  item.action === "suggestions" &&
                    "border border-[#B7DFA8] bg-[#EEF6EA] text-[#063D1E] shadow-sm hover:border-[#0F5C2E] hover:bg-[#DCEFD5]",
                )}
                key={item.label}
                onClick={() => {
                  if (item.action === "suggestions") {
      ...
      <SuggestionBox open={isSuggestionBoxOpen} onClose={() => setIsSuggestionBoxOpen(false)} />
    </>
  );
}
